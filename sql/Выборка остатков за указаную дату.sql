	...
	set @time_zone = CAST(SUBSTRING(CONVERT(char(13), GETDATE() - GETUTCDATE() ,20), 13, 1) AS INT);
		...
		N@Statement, 
		--N'INSERT INTO @TagWincc SELECT * FROM OPENQUERY(CA_103,''Tag:R,(6;7;8;9;10;11),''''2019-12-10 20:30:00'''',''''0000-00-00 00:00:00.999'''',1,261 '')  ORDER BY Timestamp ASC',
		--N'@TagWincc table (ValueId int, [Timestamp] datetime, Realvalue varchar(250),quality int,flags int)'
		N'@TagWincc table'
		,@TagWincc = @TagWincc1
